      ...
      if (localGames) {
        try {
          ...
          if (Array.isArray(parsedGames) && parsedGames.length > 0) {
            ...
            const syncedGames =
              await gamesService.syncRouletteGames(parsedGames);
  ...
  const saveGames = async (updatedGames: RouletteGame[]) => {
    ...
    if (user) {
      ...
      try {
        ...
        const gamesToAdd = updatedGames.filter(
          (g) => !currentBackendIds.has(g.id),
        );
        ...
        const gamesToRemove = currentBackendGames.filter(
          (g) => !updatedIds.has(g.id),
        );
